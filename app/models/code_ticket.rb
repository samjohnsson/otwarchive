class CodeTicket < ActiveRecord::Base
  ...
  before_validation(:on => :create) do
    raise SecurityError, "only volunteers can create code tickets" if !User.current_user.try(:support_staffer?)
  end
  ...
  attr_accessor :turn_off_watchers
  ...
  def add_default_watchers
    # on create, add owner to the watchers unless indicated otherwise
    self.watch! unless turn_off_watchers == "1"

    # TODO make default groups. e.g. coders, that people can add and remove themselves to
    # so that when tickets are created the watchers are populated with these groups.
  ...
  def stealable?
    raise SecurityError, "Couldn't check stealable. Not volunteer." unless User.current_user.try(:support_staffer?)
  ...
  def watched?
    ...
    self.code_watchers.where(:email => User.current_user.email).first
  end

  # returns an array of email addresses. [] if none
  def mail_to(private = false)
    watchers = self.code_watchers
    watchers = watchers.official if private
    watchers.map(&:email).uniq
  end

  # change the support id, and add current volunteer to watch list
  def take_and_watch!
    raise SecurityError, "not support volunteer" unless User.current_user.try(:support_staffer?)
    self.support_identity_id = User.current_user.support_identity_id
    self.watch!
  end
  ...
  def visible_code_details
    User.current_user.try(:support_staffer?) ? self.code_details : self.code_details.visible_to_all
  end
  ...
  def self.filter(params = {})
    ...
    if !params[:watching].blank?
      ...
      tickets = tickets.joins(:code_watchers).merge(CodeWatcher.where(:email => user.email))
    end
    ...
    if !params[:comments_by_support_identity].blank?
      ...
      details = details.visible_to_all unless User.current_user.try(:support_staffer?)

      tickets = tickets.joins(:code_details).merge(details)
    end
  ...
  workflow do
    ...
    on_transition do |from, to, triggering_event, *event_args|
      halt! unless User.current_user.support_staffer?
      content = "#{from} -> #{to}"
    ...
    after_transition do |from, to, triggering_event, *event_args|
      self.send_update_watchers unless [:steal].include?(triggering_event)
    end
  end
  ...
  def duplicate(original_id)
    ...
    # move all code watchers unless they are dupes
    self.code_watchers.each {|watcher| watcher.move_to_ticket(original)}
  ...
  def update_from_edit!(summary, url, browser)
    raise SecurityError, "Couldn't vote. Not support volunteer." unless User.current_user.try(:support_staffer?)
  ...
  def comment!(content, response=nil)
    ...
    if response.nil?
      response = User.current_user.support_staffer? ? "official" : "unofficial"
    end
    case response
    when "official"
      raise SecurityError, "only volunteers can comment officially" unless User.current_user.support_staffer?
      support_response = true
    when "private"
      raise SecurityError, "only volunteers can comment privately" unless User.current_user.support_staffer?
    ...
    self.send_update_watchers(private_response)
  ...
  def watch!
    ...
    self.code_watchers.create(:email => User.current_user.email, :official => User.current_user.support_staffer?)
  end

  def unwatch!
    raise "Couldn't remove watch. Not watching." unless watched?
    self.code_watchers.where(:email => User.current_user.email).destroy_all
  end

  ### SEND NOTIFICATIONS

  def send_create_watchers
    self.mail_to.each do |recipient|
      CodeTicketMailer.create_notification(self, recipient).deliver
    end
  end

  def send_update_watchers(private = false)
