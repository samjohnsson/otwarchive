class SupportTicket < ActiveRecord::Base
  ...
  # returns the support watcher if it exists, nil otherwise
  def watched?(code = nil)
    ...
    self.support_watchers.where(:email => email_to_check).first
  end

  # the current user is neither a volunteer, nor the owner of the ticket
  def public_watcher?
    User.current_user && !User.current_user.support_staffer? && (self.user != User.current_user)
  end
  ...
  def mail_to(private = false)
    watchers = self.support_watchers
    watchers = watchers.official if private
    watchers.map(&:email).uniq
  end

  def visible_support_details
    User.current_user.try(:support_staffer?) ? self.support_details : self.support_details.visible_to_all
  end
  ...
  def self.filter(params = {})
    ...
    if !params[:owned_by_user].blank?
      ...
      if !(User.current_user.try(:support_staffer?) || User.current_user == user)
    ...
    if !params[:watching].blank?
      ...
      tickets = tickets.joins(:support_watchers).merge(SupportWatcher.where(:email => User.current_user.email))
    end

    # if we haven't already checked, rule out private tickets
    if params[:owned_by_user].blank? && params[:watching].blank?
      tickets = tickets.where(:private => false) unless User.current_user.try(:support_staffer?)
    end
    ...
    if !params[:comments_by_support_identity].blank?
      ...
      tickets = tickets.joins(:support_details).merge(details)
    end
  ...
  def watch!(code = nil)
    ...
    if User.current_user
      ...
      self.support_watchers.create!(:email => User.current_user.email,
                                        :public_watcher => public_watcher?,
                                        :official =>  User.current_user.support_staffer?)
    else
      raise_unless_guest_owner(code)
      self.support_watchers.create!(:email => self.email)
    end
  end

  # remove current user or ticket owner from watchers
  def unwatch!(code = nil)
    watcher = self.watched?(code)
    raise "Couldn't remove watch. Not watching." unless watcher
    watcher.destroy
  end
  ...
  def make_private!(code=nil)
    ...
    watchers = self.support_watchers.where(:public_watcher => true)
    Rails.logger.debug "removing public watchers: #{watchers.map(&:email)}"
    watchers.delete_all
    log!("made private")
  end
  ...
  def log!(content)
    ...
    official = User.current_user && User.current_user.support_staffer? && (User.current_user != self.user)
  ...
  def raise_unless_owner_or_volunteer(code_to_test=nil)
    if code_to_test.blank?
      raise_unless_logged_in
      raise SecurityError, "neither owner nor volunteer!" unless (User.current_user.support_staffer? || self.user == User.current_user)
    else
  ...
  def raise_unless_volunteer
    raise_unless_logged_in
    raise SecurityError, "not a support volunteer!" unless User.current_user.support_staffer?
  end
