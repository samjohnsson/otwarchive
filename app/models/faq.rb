class Faq < ActiveRecord::Base
  ...
  has_many :faq_watchers  # a bunch of email addresses for update notifications
  ...
  def watched?(code = nil)
    ...
    self.faq_watchers.where(:email => email_address).first
  end
  ...
  def watch!(code = nil)
    ...
    raise_unless_logged_in_or_guest(code)
    email_address = User.current_user ? User.current_user.email : self.associated_ticket(code).email
    self.faq_watchers.create(:email => email_address)
  end
  ...
  def unwatch!(code = nil)
    ...
    raise_unless_logged_in_or_guest(code)
    email_address = User.current_user ? User.current_user.email : self.associated_ticket(code).email
    self.faq_watchers.where(:email => email_address).delete_all
  end
